// Fuentes:
// Implementacion de algoritmo Strassen obtenida de:
// https://www.geeksforgeeks.org/dsa/strassens-matrix-multiplication/
// Codigo mostrado en "[Approach 3]"
// Fecha: 29 de Agosto de 2025
// Codigo modificado para mejorar ejecuccion.
...
vector<vector<int>> resizeMatrix(vector<vector<int>> &mat, 
    ...
    for (size_t i = 0; i < mat.size(); ++i) // Cambio de int i a size_t i
        for (size_t j = 0; j < mat[0].size(); ++j) // Cambio de int i a size_t i
            resized[i][j] = mat[i][j];
...
// Recursive implementation of Strassen's matrix multiplication
// Assumes both matrices are size×size and size is a power of 2
